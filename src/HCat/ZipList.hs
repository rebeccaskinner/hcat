...
-- list is not empty.
unsafeDrop :: HasCallStack => ZipList a -> ZipList a
...
--
-- >>> cursorNext $ ZipList [] 1 [2,3,4]
-- ZipList [1] 2 [3,4]
--
-- >>> cursorNext $ ZipList [3,2,1] 4 [5,6,7]
-- ZipList [4,3,2,1] 5 [6,7]
--
-- >>> cursorNext $ ZipList [3,2,1] 4 []
-- ZipList [3,2,1] 4 []
--
cursorNext :: ZipList a -> ZipList a
cursorNext z@(ZipList _h _c []) = z
cursorNext (ZipList h c (x:xs)) = ZipList (c:h) x xs

-- | Move the value "backwards" one element (toward the head). Has no
-- effect if the cursor is already at the start of the list.
--
-- >>> cursorLast $ ZipList [3,2,1] 4 []
-- ZipList [2,1] 3 [4]
--
-- >>> cursorLast $ ZipList [3,2,1] 4 [5,6,7]
-- ZipList [2,1] 3 [4,5,6,7]
--
-- >>> cursorLast $ ZipList [] 4 [5,6,7]
-- ZipList [] 4 [5,6,7]
--
cursorLast :: ZipList a -> ZipList a
cursorLast z@(ZipList [] _c _t) = z
cursorLast (ZipList (x:xs) c t) = ZipList xs x (c:t)
